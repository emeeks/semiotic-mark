  ...
  _createClass(Mark, [{
    ...
    value: function updateSketchy(nextProps) {
      ...
      if (sketchyHash && sketchyHash !== this.state.sketchyHash) {
        ...
        var roughGenerator = (0, _roughjs.generator)({}, { width: 1000, height: 1000 });
        ...
        switch (nextProps.markType) {
          case "line":
            drawingInstructions = roughGenerator.line(nextProps.x1, nextProps.y1, nextProps.x2, nextProps.y2, roughOptions);
            break;
          case "rect":
            if (nextProps.rx || nextProps.ry) {
              drawingInstructions = roughGenerator.circle(nextProps.x, nextProps.y, nextProps.width, roughOptions);
            } else {
              drawingInstructions = roughGenerator.rectangle(nextProps.x, nextProps.y, nextProps.width, nextProps.height, roughOptions);
            }
            break;
          case "circle":
            drawingInstructions = roughGenerator.circle(nextProps.cx, nextProps.cy, nextProps.r * 2, roughOptions);
            break;
          case "ellipse":
            drawingInstructions = roughGenerator.ellipse(nextProps.x, nextProps.y, nextProps.width, nextProps.height, roughOptions);
            break;
          case "polygon":
        ...
        var roughPieces = [];
        roughGenerator.toPaths(drawingInstructions).forEach(function (_ref) {
          var d = _ref.d,
              fill = _ref.fill,
              stroke = _ref.stroke,
    ...
    value: function shouldComponentUpdate(nextProps) {
      ...
      if (nextProps.renderMode || this.props.renderMode || this.props.markType !== nextProps.markType || this.state.dragging || this.props.forceUpdate || nextProps.forceUpdate || this.props.className !== nextProps.className || this.props.children !== nextProps.children) {
        return true;
      }
